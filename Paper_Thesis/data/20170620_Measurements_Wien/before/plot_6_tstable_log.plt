...
set title "IV-Curve SiPM #6 (Logscale)"
...
pol(x) = ( x > 30.67 && x < 32 ) ? a * x**2 + b * x + c : 1/0
lin(x) = ( x > 6 && x < 33 ) ? m * x + y0 : 1/0
...
fit [10:30.5] lin(x) "iu_curve_Wien_sipm_6_tstable.txt" using 2:(log($3)) via m,y0
fit [31:32] pol(x) "iu_curve_Wien_sipm_6_tstable.txt" using 2:(log($3)) via a,b,c
