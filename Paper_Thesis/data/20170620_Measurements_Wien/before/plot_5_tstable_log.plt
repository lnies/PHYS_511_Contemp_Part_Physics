...
set title "IV-Curve SiPM #5 (Logscale)"
...
pol(x) = ( x > 70 && x < 72 ) ? a * x**2 + b * x + c : 1/0
lin(x) = ( x > 27 && x < 72 ) ? m * x + y0 : 1/0
...
fit [48:69] lin(x) "iu_curve_Wien_sipm_5_tstable.txt" using 2:(log($3)) via m,y0
fit [70:72] pol(x) "iu_curve_Wien_sipm_5_tstable.txt" using 2:(log($3)) via a,b,c
