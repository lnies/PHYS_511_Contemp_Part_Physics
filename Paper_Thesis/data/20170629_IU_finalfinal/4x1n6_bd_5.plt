...
set style line 1 lc "cyan" lt 1 lw 3 pt 9 ps 3 pi 2
...
fit [x_min1:x_max1] pol1(x) "iu_curve_4x1n6_5gC.txt" using 2:(log10($3-yoffset)) via m,y0
fit [x_min2:x_max2] pol2(x) "iu_curve_4x1n6_5gC.txt" using 2:(log10($3-yoffset)) via A,B,C
...
if (x1<x2) set label 1 sprintf("U_{BD} = (%2.4f {/Symbol \261} %2.4f)V",x1,x1_err) at x1+1,y1+1.2 font "Arial, 28"; \
	else set label 2 sprintf("U_{BD} = (%2.4f {/Symbol \261} %2.4f)V",x2,x2_err) at x2+1,y1+1.2 font "Arial, 28"

if (x1<x2) set arrow 1 from x1+1,y1+1 to x1+0.1,y1+0.1 lw 3 front ; \
	else set arrow 1 from x2+1,y1+1 to x2+0.1,y1+0.1 lw 3 front



plot \
	"iu_curve_4x1n6_5gC.txt" using 2:(log10($3-yoffset)) with lp ls 1 title "5 {/Symbol \260}C", \
